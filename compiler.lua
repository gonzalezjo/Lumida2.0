...
do
  ...
  function names:random()
	...
	function compile_to_proto(source, name)
		...
		local zio = luaZ:init(luaZ:make_getF(source), nil)
		if not zio then return end
		return luaY:parser(LuaState, zio, nil, "@"..name)
	end

	function compile_proto(func)
		local writer, buff = luaU:make_setS()
		-- luaU:dump builds a binary chunk
		luaU:dump(LuaState, func, writer, buff)
		return buff.data
...
return { compile = compile, compile_to_proto = compile_to_proto, compile_proto = compile_proto }